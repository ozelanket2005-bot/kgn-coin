...
function init() {
    ...
    else document.getElementById('display-name').innerText = "👤 " + state.username;
...
function renderTasks() {
    ...
    const tasksHTML = [
        { id: 'gold', title: '10 Reklam İzle', desc: 'Reklam başı 100 KGn', cooldown: state.tasks.gold.nextAvailable },
        { id: 'click', title: '2 Reklam İzle', desc: '1 Saat 2x Tık', cooldown: state.tasks.click.nextAvailable },
        { id: 'energy', title: '2 Reklam İzle', desc: 'Enerji Fullenir', cooldown: state.tasks.energy.nextAvailable }
    ];
    list.innerHTML = tasksHTML.map(t => {
        let isWaiting = Date.now() < t.cooldown;
